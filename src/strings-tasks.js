...
function removeLastOccurrences(str, value) {
  const subIndex = str.lastIndexOf(value);
  if (str.lastIndexOf(value) !== -1)
    return str.substring(0, subIndex) + str.substring(subIndex + value.length);
  return str;
}
...
function isPalindrome(str) {
  const lowerStr = str.toLowerCase().replaceAll(/\W/g, '');
...
function findLongestWord(sentence) {
  const sentenceArr = sentence.replaceAll(/[^\sA-Za-z0-9]/g, '').split(' ');
  let maxLen = -1;
  let maxWord;
  for (let i = 0; i < sentenceArr.length; i += 1) {
    const currWord = sentenceArr[i];
    const currLen = currWord.length;
    if (currLen > maxLen) {
      maxLen = currLen;
      maxWord = currWord;
    }
...
function reverseWords(str) {
  const strArr = str.replaceAll(/[^\sA-Za-z0-9]/g, '').split(' ');
...
function encodeToRot13(str) {
  return str.replace(
    ...
    (ch) =>
      'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[
        'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.indexOf(ch)
      ]
  );
}
